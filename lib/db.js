...
export async function recordAmapUsage(service) {
  ...
  await query(
    `INSERT INTO amap_usage_daily (usage_date, service, count, updated_at)
     VALUES ((NOW() AT TIME ZONE 'Asia/Shanghai')::date, $1, 1, NOW())
     ON CONFLICT (usage_date, service)
     DO UPDATE SET count = amap_usage_daily.count + 1, updated_at = NOW()`,
    [service],
  )
}
