...
async function ensureAmapUsageDaily() {
  await query(`
    CREATE TABLE IF NOT EXISTS amap_usage_daily (
      usage_date DATE NOT NULL,
      service    TEXT NOT NULL CHECK (service IN ('map_init', 'place_search')),
      count      INTEGER NOT NULL DEFAULT 0 CHECK (count >= 0),
      updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      PRIMARY KEY (usage_date, service)
    )
  `)
  await query(`CREATE INDEX IF NOT EXISTS idx_amap_usage_daily_service_date ON amap_usage_daily (service, usage_date DESC)`)
  await query(`ALTER TABLE amap_usage_daily ENABLE ROW LEVEL SECURITY`)
}
...
async function initializeDB() {
  ...
  const existingTables = await query(`
    ...
    WHERE table_schema = 'public'
      AND table_name IN ('city_addresses', 'app_settings', 'app_users', 'registration_whitelist', 'map_search_logs', 'amap_usage_daily')
  `)
  ...
  if (hasCoreTables) {
    ...
    if (!existingTableNames.has('amap_usage_daily')) await ensureAmapUsageDaily()
...
export async function recordAmapUsage(service, amount = 1) {
  if (!['map_init', 'place_search'].includes(service)) throw new Error('无效的高德服务类型')
  const increment = Math.max(1, Math.min(Math.trunc(Number(amount) || 1), 100))
  await initDB()
  await query(
    `INSERT INTO amap_usage_daily (usage_date, service, count, updated_at)
     VALUES ((NOW() AT TIME ZONE 'Asia/Shanghai')::date, $1, $2, NOW())
     ON CONFLICT (usage_date, service)
     DO UPDATE SET count = amap_usage_daily.count + EXCLUDED.count, updated_at = NOW()`,
    [service, increment],
  )
}
...
export async function getSystemSettings() {
  ...
  const result = await query(`
    ...
    WHERE key IN (
      ...
      'amap_map_monthly_quota', 'amap_search_monthly_quota'
    )
  `)
  const map = Object.fromEntries(result.rows.map(row => [row.key, row.value]))
  ...
  return {
    ...
    amapMapMonthlyQuota: Math.max(0, Number(map.amap_map_monthly_quota || 0) || 0),
    amapSearchMonthlyQuota: Math.max(0, Number(map.amap_search_monthly_quota || 0) || 0),
  }
}
